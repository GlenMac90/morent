    ...
    <footer className="flex flex-col pt-12 md:bg-white md:px-[3.75rem] md:pb-[3.75rem] md:pt-[5rem]">
      <div className="flex flex-col md:flex-row md:justify-between">
        ...
        <div className="flex flex-col justify-start gap-4">
          ...
          <p className="text-[0.75rem] font-medium not-italic leading-6 tracking-[-0.0075rem] text-gray400 md:text-[1rem] md:leading-8 md:tracking-[-0.01rem] md:text-[#13131399]">
        ...
        <div className="flex flex-row gap-[4.06rem] md:gap-6 lg:mr-[3.75rem] lg:gap-[3.75rem]">
          ...
          <div className="mt-12 flex flex-col gap-12 min-[500px]:flex-row md:mt-0 md:gap-6 lg:gap-[3.75rem]">
            {footerLinks.slice(0, 2).map((item) => (
              <div key={item.title}>
                <h3 className="text-[1.25rem] font-semibold not-italic leading-6 text-gray900 md:text-gray800">
                  {item.title}
                </h3>
                <div className="mt-5 flex flex-col gap-4 sm:mt-6">
                  {item.links.map((link) => (
                    <Link
                      key={link.title}
                      href={link.url}
                      className="text-[1rem] font-medium not-italic leading-[1.2rem] text-gray400 md:text-[#13131399]"
                    >
                      {link.title}
                    </Link>
                  ))}
      ...
      <Separator className="hidden sm:mt-12 sm:block sm:bg-blue50 md:mt-[3.75rem]" />
      {/* ©2023 MORENT. All rights reserved Privacy & Policy, Terms & Condition */}
      <div className="mt-12 flex flex-col text-[0.75rem] font-semibold not-italic leading-6 tracking-[-0.0075rem] text-gray900 sm:flex-row-reverse sm:justify-between md:text-[1rem] md:leading-8 md:tracking-[-0.01rem] md:text-gray800">
